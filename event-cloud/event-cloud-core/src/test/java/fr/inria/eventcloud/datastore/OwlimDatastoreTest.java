...
public class OwlimDatastoreTest {
    ...
    public void testAddStatementsMultiThread() {
        ...
        Assert.assertEquals(CONCURRENT_ADD_OPERATIONS, sparqlConstruct(
                context,
    ...
    public void testRemoveAllStatements() {
        ...
        Assert.assertEquals(0, sparqlConstruct(
                context,
                "CONSTRUCT { ?s ?p ?o } WHERE { GRAPH<" + context
                        + "> { ?s ?p ?o } . }").size());
    }
    ...
    public void testAddStatementsSingleThread() {
        for (int i = 0; i < statementsForSingleThread.length; i++) {
            datastore.addStatement(
                    context, RDF2GoBuilder.createStatementInternal(
                            statementsForSingleThread[i][0],
                            statementsForSingleThread[i][1],
                            statementsForSingleThread[i][2]));
        }
        Assert.assertEquals(statementsForSingleThread.length, sparqlConstruct(
                context,
                "CONSTRUCT { ?s ?p ?o } WHERE { GRAPH<" + context
                        + "> { ?s ?p ?o } . }").size());
    }
    ...
    public void testSparqlConstructQueries() {
        Assert.assertEquals(
                3, sparqlConstruct(
                        context,
                        "CONSTRUCT { ?s <" + statementsForSingleThread[0][1]
                                + "> ?o } WHERE { GRAPH<" + context
                                + "> { ?s <" + statementsForSingleThread[0][1]
                                + "> ?o } . }").size());

        Assert.assertEquals(2, sparqlConstruct(
                context,
                "CONSTRUCT { ?s ?p <" + statementsForSingleThread[0][2]
                        + "> } WHERE { GRAPH<" + context + "> { ?s ?p <"
                        + statementsForSingleThread[0][2] + "> } . }").size());

        Assert.assertEquals(
                1, sparqlConstruct(
                        context,
                        "CONSTRUCT { <" + statementsForSingleThread[2][0]
                                + "> ?p ?o } WHERE { GRAPH<" + context
                                + "> { <" + statementsForSingleThread[2][0]
                                + "> ?p ?o } . }").size());
    }

    public void testAskQueryWithData() {
        Assert.assertEquals(true, datastore.sparqlAsk(context, "ASK { GRAPH<"
                + context + "> { ?s ?p ?o } . }"));
    }

    public void testRemoveStatement() {
        datastore.removeStatement(context, new StatementImpl(
                context, new URIImpl(statementsForSingleThread[0][0]),
                new URIImpl(statementsForSingleThread[0][1]), new URIImpl(
                        statementsForSingleThread[0][2])));

        Assert.assertEquals(
                statementsForSingleThread.length - 1, sparqlConstruct(
                        context,
                        "CONSTRUCT { ?s ?p ?o } WHERE { GRAPH<" + context
                                + "> { ?s ?p ?o } . }").size());
    }
    ...
    public void testAskQueryWithoutData() {
        ...
        Assert.assertEquals(false, datastore.sparqlAsk(context, "ASK { GRAPH<"
                + context + "> { ?s ?p ?o} . }"));
    }
    ...
    public void testSparqlConstructWithFilter() {
        ...
        for (int i = 0; i < statementsForSingleThread.length; i++) {
            datastore.addStatement(context, new StatementImpl(
                    context, new URIImpl(statementsForSingleThread[i][0]),
                    new URIImpl(statementsForSingleThread[i][1]), new URIImpl(
                            statementsForSingleThread[i][2])));
        }
    ...
    public void testSparqlSelect() {
        ...
        qtr =
                datastore.sparqlSelect(context, "SELECT ?s WHERE { GRAPH<"
                        + context + "> { ?s ?p ?o } . }");
