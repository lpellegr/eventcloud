 ...
 * {@link SparqlResponse} by using the original SPARQL request. Indeed, due to the
 * infrastructure, we can route only a sub-set of SPARQL queries. This mean that
 * the {@link SparqlResponse} does not contains the final result. To get the final
 * result we have to use a {@link SparqlColander} to remove some extra values.
 * 
 * @author lpellegr
 */
public class SparqlColander {
	...
	/**
	 * Filters a set of {@link SparqlResponse} for a given {@code originalAskQuery}.
	 * 
	 ...
	 */
    public synchronized boolean filterSparqlAsk(String originalAskQuery, List<SparqlResponse> responses) {
    	this.cleanAndFill(this.models[0], responses);
    	return this.models[0].sparqlAsk(originalAskQuery);
    }
    
    /**
	 * Filters a set of {@link SparqlResponse} for a given {@code originalConstructQuery}.
	 * 
	 ...
	 */
    public synchronized ClosableIterable<Statement> filterSparqlConstruct(String originalConstructQuery, List<SparqlResponse> responses) {
    ...
    /**
	 * Filters a set of {@link SparqlResponse} for a given {@code originalDescribeQuery}.
	 * 
	 ...
	 */
    public synchronized ClosableIterable<Statement> filterSparqlDescribe(String originalDescribeQuery, List<SparqlResponse> responses) {
    	this.cleanAndFill(this.models[2], responses);
    	return this.models[2].sparqlConstruct(originalDescribeQuery);
    }
    
    /**
	 * Filters a set of {@link SparqlResponse} for a given {@code originalSelectQuery}.
	 * 
	 ...
	 */
    public synchronized QueryResultTable filterSparqlSelect(String originalSelectQuery, List<SparqlResponse> responses) {
