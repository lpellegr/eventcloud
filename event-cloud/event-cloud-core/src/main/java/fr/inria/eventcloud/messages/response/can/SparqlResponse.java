...
public abstract class SparqlResponse extends AnycastResponse {
    ...
    public synchronized List<ClosableIterableWrapper> getDeserializedResults() {
        List<ClosableIterableWrapper> deserializedResults =
                new ArrayList<ClosableIterableWrapper>();
        for (Byte[] value : this.serializedResults) {
            try {
                deserializedResults.add((ClosableIterableWrapper) ByteToObjectConverter.ObjectStream.convert(toPrimitive(value)));
            } catch (IOException e) {
                e.printStackTrace();
        ...
        return new AnycastResponseRouter<AnycastResponse>() {
            ...
            public void makeDecision(StructuredOverlay overlay,
                ...
                Future<ClosableIterableWrapper> result =
                        ((SparqlRequestResponseManager) overlay.getRequestResponseManager()).getPendingRequestsResult()
                                .remove(response.getId());
                // result != null <-> we are on a peer validating the
                // constraints
                if (result != null) {
                    // ensures that the query datastore has terminated
                    // before to send back the request
                    try {
                        ((SparqlResponse) response).getSerializedResults()
                                .add(
