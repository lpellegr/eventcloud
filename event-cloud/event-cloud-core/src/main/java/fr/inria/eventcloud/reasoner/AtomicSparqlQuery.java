...
public class AtomicSparqlQuery {
	...
	public AtomicSparqlQuery(String[] tripleElements) {
		...
		if (tripleElements.length != 3) {
			throw new IllegalArgumentException("An atomic sparql must have three elements!");
		}
		
		for (String tripleElt : tripleElements) {
			if (tripleElt == null) {
				throw new IllegalArgumentException("An atomic sparql element can not be null!");
			}
		}
	...
	public String[] toArray() {
		return new String[] {
			isVariable(this.tripleElements[0]) ? null : this.tripleElements[0],
			isVariable(this.tripleElements[1]) ? null : this.tripleElements[1],
			isVariable(this.tripleElements[2]) ? null : this.tripleElements[2]
		};
	}
