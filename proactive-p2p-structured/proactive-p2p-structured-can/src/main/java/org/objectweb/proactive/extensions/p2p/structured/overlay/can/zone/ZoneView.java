 ...
 * @author lpellegr
 *
 * @param <C> the {@link Coordinate} type.
...
public abstract class ZoneView<C extends Coordinate<E, T>, E extends Element<T>, T> implements Serializable {
     ...
     * by any view on any dimension. This semantic is used to construct 
     * systems in which queries can reach several peers.
     * 
     * @param dimension
     *            the dimension.
     *            
     ...
     *         {@code -1} if the coordinate is taller than the lower
     *         bound of the zone view and {@code 1} if the coordinate is
     *         greater or equal to the upper bound of the zone view.
     */
    ...
    /**
     * Returns a boolean indication whether the given {@code zone} overlaps 
    ...
    /**
     * Returns a boolean indicating whether the given {@code view} overlaps 
     * the current zone. The specified {@code view} must overlap on all 
     * dimensions.
     *
     * @param view 
     *              the zone view to compare with.
     *              
     * @return {@code true} if the specified zone view overlaps the current 
     *         zone view on all dimensions, {@code false} otherwise.
     */
    ...
    /**
     * Checks whether the given {@code view} abuts the current one in 
     * the given {@code dimension} and {@code direction}.
     *
     * @param view 
     ...
     *              {@code false} is the inferior direction and {@code true} 
     *              is the superior direction.
     *              
     * @return a boolean indicating if the specified {@code view} abuts 
     *         the current zone.
     */
    public boolean abuts(ZoneView<C,E,T> view, int dimension, boolean direction) {
        return (direction && 
                    (this.lowerBound.getElement(dimension).compareTo(
                            view.getUpperBound(dimension)) == 0)) 
               || (!direction && (this.upperBound.getElement(dimension).compareTo(
                                view.getLowerBound(dimension)) == 0));
    }
    
    /**
     * Returns the dimension on which the given {@code view} neighbors 
     * the current one. The result is the dimension number or {@code -1}
     * if the specified view does not neighbor the current view.
     * <p>
     * In a d-dimensional space, two views are neighbors if their edges overlap 
     * in exactly {@code d-1} dimensions and abut in exactly {@code 1} dimension.
     *
     * @param view 
     *              the zone view to compare with.
     * 
     * @return the dimension on which the given {@code view} neighbors 
     *         the current one.
     */
    ...
    /**
     * Returns two views representing the original one split into two 
     * following the specified {@code dimension}.
     * 
     * @param dimension
     *            the dimension to split on.
     *            
     * @return two new views standing for the original one split into 
     *         two following the specified {@code dimension}.
     */
	@SuppressWarnings("unchecked")
    public Pair<ZoneView<C,E,T>> split(int dimension) {
    	Element<T> middle = Element.middle(
    	...
    	try {
			...
			return new Pair<ZoneView<C,E,T>>(
						this.createZoneView(this.lowerBound, (C) upperBoundCopy),
						this.createZoneView((C) lowerBoundCopy, this.upperBound));
    ...
    public ZoneView<C,E,T> merge(ZoneView<C,E,T> view) {
        ...
        try {
            ...
            lowerBoundCopy.setElement(d, (E) Element.min(this.lowerBound.getElement(d), view.getLowerBound(d)));
            upperBoundCopy.setElement(d, (E) Element.max(this.upperBound.getElement(d), view.getUpperBound(d)));
	...
	/**
	 * Creates a new {@link ZoneView} with the specified {@code lowerBound} 
	 * and {@code upperBound} by using the same concrete class type as the 
	 * current one.
	 * 
	 * @param lowerBound the lower bound coordinate.
	 * @param upperBound the upper bound coordinate.
	 * 
	...
	private ZoneView<C, E, T> createZoneView(C lowerBound, C upperBound) {
    	...
    	try {
			result = this.getClass().getConstructor(
			            lowerBound.getClass(), upperBound.getClass())
