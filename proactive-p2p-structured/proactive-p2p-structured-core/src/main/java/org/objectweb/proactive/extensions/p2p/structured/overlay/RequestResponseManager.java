...
public abstract class RequestResponseManager implements Serializable {
    ...
    private Map<UUID, ResponseEntry> repliesReceived = new ConcurrentHashMap<UUID, ResponseEntry>();
    ...
    public Response<?> dispatch(Request<?> request) throws DispatchException {
    	 if (logger.isDebugEnabled()) {
             logger.debug("Dispatching " + request.getClass().getSimpleName() + " request " 
            		 		+ request.getId() + " from " + this.overlay.toString());
         }
    ...
    protected Response<?> pullResponse(UUID requestId) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Final response received for request " + requestId + " on " + this.overlay);
        }
    ...
    protected void waitForFinalResponse(UUID requestId) {
        if (logger.isDebugEnabled()) {
            ...
            log.append(this.repliesReceived.get(requestId).getExpectedResponsesCount());
        ...
        synchronized (entry) {
            while (entry.getStatus() 
                        != ResponseEntry.Status.RECEIPT_COMPLETED) {
                try {
                    entry.wait();
	...
	/**
	 * Pushes the final response and notify the initial sender that a new response has
	 * been received. The notification is done to remove the synchronization
	 * point (which has been previously set by {@link #waitForFinalResponse(UUID)}).
	 * 
	 * @param response
